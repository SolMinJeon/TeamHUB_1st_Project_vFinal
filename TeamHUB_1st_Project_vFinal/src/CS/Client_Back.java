...
class Client_Back extends Thread {
	...
	public void getUserInfo(String NickName, String IPAddress, int Port) {
		...
		this.NickName = LoginPanel.lname;
		this.IPAddress = "172.16.10.42";
		this.Port = 8080;
	}

	public void setGui(Client_F chatgui) {
...
//					chatgui.AppendUserList("손님");
				} else if (Message.contains("이 입장하셨습니다.")) {
					// ~~ 님이 입장하셨습니다. 라는 식별자를 받으면 기존의 닉네임 리스트 초기화 후 새로 입력시킵니다.
					NickNameList.clear();
					chatgui.UserList.setText(null);
					chatgui.AppendMessage(Message);
				} else if (Message.contains("이 퇴장하셨습니다.")) {
