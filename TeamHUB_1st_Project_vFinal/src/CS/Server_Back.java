...
public class Server_Back extends Thread {
	...
	public void run() {
		try {
			NickNameList.add("관리자"); // 유저목록의 첫 번째 서버(Admin)을 추가합니다.
			while (true) {
	...
	class ReceiveInfo extends Thread { 
		...
		public void run() {
			try {
				...
				serverchatgui.UserList.setText(null); 
				serverchatgui.AppendUserList(NickNameList);
				Transmitall(NickName + "이 입장하셨습니다.\n");
				...
				System.out.println(NickName + "이 퇴장하셨습니다.");
				removeClient(this, NickName);
				Transmitall(NickName + "이 퇴장하셨습니다.\n");
				for (int i = 0; i < NickNameList.size(); i++) {
					Transmitall("+++닉네임의시작+++" + NickNameList.get(i));
				}
				serverchatgui.AppendMessage(NickName + "이 퇴장하셨습니다.\n");
			}
		}
